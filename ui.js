...
function settingsSection(state, packs) {
  const maxSilid = state.settings.playersCount <= 5 ? 1 : state.settings.playersCount <= 8 ? 2 : 3;
  ...
  <section class="card">
    ...
    <div class="grid-2">
      ...
      <div><label>Jumlah Silid (1-${maxSilid})</label><input id="silidCount" type="number" min="1" max="${maxSilid}" value="${state.settings.silidCount}" /></div>
      <div><label>Jumlah Kepiting (0-${kepMax})</label><input id="kepitingCount" type="number" min="0" max="${kepMax}" value="${state.settings.kepitingCount}" /></div>
  ...
  <section class="card">
    <button class="btn btn-primary" id="startMatchBtn">Mulai Ronde</button>
  </section>`;
}
...
function renderReveal(state) {
  ...
  const roleLabel = assignment?.role === "Kepiting" ? "KAMU KEPITING 🦀" : assignment?.role?.toUpperCase();
...
function renderVoting(state, candidates) {
  const tieText = state.round.voting.tieCandidates
    ? `<p class="warn">Revote antar kandidat seri: ${state.round.voting.tieCandidates.map((id) => esc(state.players.find((p) => p.id === id)?.name || "-")).join(", ")}</p>`
    : "";
...
function renderElimination(state) {
  ...
  <section class="card">
    ...
    ${a?.role === "Silid" ? `<p class="win">Satu Silid telah tertangkap!</p>` : ""}
    ${a?.role === "Kepiting" ? `
      <div class="card">
...
function renderLeaderboard(state) {
  ...
  <table class="table">
    ...
    <tbody>
      ${rows.map((p, idx) => `<tr><td>${idx + 1}</td><td>${esc(p.name)}</td><td>${p.score}</td><td>W:${p.stats.asWarga} S:${p.stats.asSilid} K:${p.stats.asKepiting} B:${p.stats.bonusHits}</td></tr>`).join("")}
    </tbody>
  </table>`;
}
...
export function renderApp(state, packs, totalWordCount) {
  ...
  const header = `
    <header class="topbar">
      <h1 class="title">🦀 Silid Kepiting Game</h1>
